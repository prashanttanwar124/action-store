...
const form = useForm({
  ...
  tag: props.slider.tag || '',
  cta_text: props.slider.cta_text || '',
  link_url: props.slider.link_url || '',
  photo_label: props.slider.photo_label || '',
  bg_color: props.slider.bg_color || '#1a1a1a',
...
<template>
  <AdminLayout :title="'Edit Slide: ' + (slider.title.replace(/\n/g, ' ') || 'Hero Banner')">
      ...
      <form @submit.prevent="submit" class="bg-white border border-[#e0d9cc] rounded-3xl p-6 sm:p-8 shadow-2xs space-y-6">
        ...
        <div class="border-b border-[#e0d9cc]/60 pb-5">
          <h2 class="text-xl font-serif font-medium text-[#1d1d1f]">
            Edit Slide #{{ slider.id }}
          </h2>
          <p class="text-xs text-[#6e6e73] mt-0.5">
            Modify copywriting, target URL, and appearance settings.
          </p>
        </div>
          ...
          <div class="md:col-span-2 space-y-3 pt-2 border-t border-[#e0d9cc]/60">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <span class="text-xs font-semibold text-stone-800">Upload New Image File</span>
